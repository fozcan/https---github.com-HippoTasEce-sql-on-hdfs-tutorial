...
Enterprises are increasingly use Apache Hadoop, more specifically HDFS, as a central repository for all their data;
...
At the same time many enterprise data management tools (e.g. from SAP ERP to Tableau) rely on SQL and many enterprise users are familiar and comfortable with it.
...
There is a wide variety of solutions, system architectures and capabilities in this space.
This tutorial tries to shed a light on this matter.
...
\item{ {\bf Native systems with MPP architectures:} These are systems that employ an MPP database architecture that uses stand-alone long-running processing to execute SQL queries over HDFS data. This category includes systems such as Impala, Presto, Drill, and Big SQL}
...
An important aspect of SQL-on-Hadoop, which is orthogonal to all these categories, is the various file formats used to store the data. While text is an important format for in-situ SQL analysis, and ELT processing, various columnar formats, such as sequence, Avro, ORC, and Parquet, have been proposed to provide fast analytics query processing. 

In this tutorial, we will first introduce these file formats, then we will discuss the characteristics, the pros and cons of each category in detail. In particular, we will examine their architectural differences, and cover in-depth their approach to indexing, updates, query optimization, as well as new algorithms they introduce in a 3-hour tutorial.
...
\noindent{\bf Text}. By far the most popular file format is of course plain text.
